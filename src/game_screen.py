...
class GameScreen:
    def __init__(self, application, players_boards, button_grid, counter_own, counter_opp, hidden=True):
        ...
        self.__setup_ui(button_grid, counter_own, counter_opp)
        if hidden:
            self.hide()
        else:
            self.show()
    ...
    def __setup_ui(self, button_grid, counter_own, counter_opp):
        self.button_grid = button_grid
        self.button_grid.add_function(GameState.PLAYER1_PLAYING, lambda i, j: self.__button_pressed(i, j, 1))
        self.button_grid.add_function(GameState.PLAYER2_PLAYING, lambda i, j: self.__button_pressed(i, j, 0))
    ...
    def __button_pressed(self, i, j, player):
    ...
    def __handle_shooting(self):
        if self.selected_tile is not None:
            ...
            if new_tile_state == TileState.EMPTY_SHOT:  # missed
                ...
                size = self.__check_if_ship_destroyed(board, s_i, s_j)
                if size is not False:  # destroyed entire ship
                    ...
                    self.__shoot_sides_of_ship(board, s_i, s_j)
    ...
    def __check_if_ship_destroyed(self, board, i, j):
        sizes = []
        for (di, dj) in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            size = self.__check_side(board, i, j, di, dj)
    ...
    def __shoot_sides_of_ship(self, board, i, j):
        self.__shoot_sides_of_tile(board, i, j)
        for (di, dj) in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            size = 0
            while utils.in_bound(i + size * di + di, j + size * dj + dj) and \
                    board[i + size * di + di][j + size * dj + dj] == TileState.SHIP_SHOT:
                self.__shoot_sides_of_tile(board, i + size * di + di, j + size * dj + dj)
                size += 1

    def __shoot_sides_of_tile(self, board, i, j):
        for ddi in (-1, 0, 1):
            for ddj in (-1, 0, 1):
                if utils.in_bound(i + ddi, j + ddj) and \
                        board[i + ddi][j + ddj] == TileState.EMPTY:
                    board[i + ddi][j + ddj] = TileState.EMPTY_SHOT
                    self.button_grid.set_button_state(i + ddi, j + ddj, board[i + ddi][j + ddj])
