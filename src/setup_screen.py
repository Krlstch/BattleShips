...
class SetupScreen:
    ...
    def check_ship_placement(self, i, j, player):
        ...
        if board[i][j] == TileState.EMPTY:
            return self.add_ship(i, j, board)
        else:  # board[i][j] == TileState.SHIP:
            return self.remove_ship(i, j, board)
    ...
    def add_ship(self, i, j, board):
        ...
        for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            sizes.append(self.check_side(board, i, j, di, dj))

        if sizes[0] + sizes[1] != 0 and sizes[2] + sizes[3] != 0:
            return False, False  # would create a curved ship

        return [sum(sizes) + 1], sizes

    def remove_ship(self, i, j, board):
        sizes = []
        for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            sizes.append(self.check_side(board, i, j, di, dj))
